...
<mark>（T1,T2右手螺旋向外，F沿L_0，T_p垂直于L_0，右手螺旋向外）</mark>
...
function K = get_k_length(leg_length, dt)
    ...
    R = 0.06;            % 驱动轮半径
    l = 0.02363;         % 机体重心距离其转轴距离
    m_w = 0.553;         % 驱动轮质量
    m_p = 2.909;         % 摆杆质量
    M = 11.427;          % 机体质量
    I_w = 0.000418255;   % 驱动轮转动惯量
    I_M = 0.407431240;   % 机体绕质心转动惯量
    g = 9.8;             % 重力加速度

    % ============================= 1.2 计算 ============================= %

    %此处理想化
    L = leg_length/2;                               % 摆杆重心到驱动轮轴距离
    L_M = leg_length/2;                             % 摆杆重心到其转轴距离
    I_p = (1/12) * m_p * ((L + L_M)^2 + 0.05^2);    % 摆杆转动惯量
...
###### 1.1 <mark>机械给定$R,l,m_w,m_p,M,I_w,I_M,g$</mark>
