...
| $T_1$ | 后关节力矩  | 右手螺旋向外(逆时针)               | $N·m$ |
| $T_4$ | 前关节力矩  | 右手螺旋向外(逆时针)               | $N·m$ |

## 1. 正解算：得到$\phi_2,\phi_3$，腿长$L_0$和腿摆角$\phi_0$
...
        $\phi_0=arctan(\frac{y_C}{x_C-\frac{l_5}{2}})$，也可以用$arccos(\frac{x_C-\frac{l_5}{2}}{L_0})$
...
模型见Model文件夹下 2.Publish_InvertedPendulum_VMC

视频见Video文件夹下 2.Publish_InvertedPendulum_VMC
...
| $\alpha$ | 机体竖直方向与摆杆的夹角，等于$\frac{\pi}{2}-\phi_0$ | 逆时针 | $rad$ |
...
            $T=-K_{11}(\theta-\theta_d)-K_{12}\theta'-K_{13}x-K_{14}(x'-x_d')-K_{15}(\phi-\phi_d)-K_{16}\phi'$

            $T_p=-K_{21}(\theta-\theta_d)-K_{22}\theta'-K_{23}x-K_{24}(x'-x_d')-K_{25}(\phi-\phi_d)-K_{26}\phi'$

###### 4.2 <mark>根据机械结构调整力矩</mark>

## Simscape验证

模型见Model文件夹下 2.Publish_InvertedPendulum_LQR

效果见Video文件夹下 2.Publish_InvertedPendulum_LQR
...
> 
> 5. [【RM2025 个人开源】山海机甲平衡步兵完全开源](https://bbs.robomaster.com/article/810688?source=1)
